...
void buildChainSingleMass(TChain* bkgMC, TString channel, int sampleIndex, int mh) {
  ...
  if(sampleIndex==0){
    //7TeV
    if(useSqrts==1)bkgMC->Add(filePath7TeV + "/" + chPath +"/HZZ4lTree_H"+strM+"_withSMD.root");
    else     bkgMC->Add(filePath8TeV + "/" + chPath +"/HZZ4lTree_H"+strM+"_withSMD.root");
    ...
    bkgMC->Add(filePath7TeV + "/" + chPath +"/HZZ4lTree_ZZTo4tau_withSMD.root");
      ...
      //8TeV
      bkgMC->Add(filePath8TeV + "/" + chPath +"/HZZ4lTree_ZZTo2e2mu_withSMD.root");
      bkgMC->Add(filePath8TeV + "/" + chPath +"/HZZ4lTree_ZZTo2e2tau_withSMD.root");
      bkgMC->Add(filePath8TeV + "/" + chPath +"/HZZ4lTree_ZZTo2mu2tau_withSMD.root");
      bkgMC->Add(filePath8TeV + "/" + chPath +"/HZZ4lTree_ZZTo4e_withSMD.root");
      bkgMC->Add(filePath8TeV + "/" + chPath +"/HZZ4lTree_ZZTo4mu_withSMD.root");
      bkgMC->Add(filePath8TeV + "/" + chPath +"/HZZ4lTree_ZZTo4tau_withSMD.root");
    }
    ...
    bkgMC->Add(filePath8TeV + "/" + chPath +"/HZZ4lTree_ggZZ2l2l_withSMD.root");
    bkgMC->Add(filePath8TeV + "/" + chPath +"/HZZ4lTree_ggZZ4l_withSMD.root");
    }
     ...
     if(useSqrts==1){   //7TeV
      ...
      bkgMC->Add(filePath7TeVPS + "/" + chPath +"/HZZ4lTree_jhuPseH"+strM+"_withSMD.root");
    }
    else{   //8TeV
      bkgMC->Add(filePath8TeVPS + "/" + chPath +"/HZZ4lTree_jhuPseH"+strM+"_withSMD.root");
    }
  }
...
//===================================================
TH1F *fillKDhisto(TString channel, int sampleIndex,float mzzLow,float mzzHigh){
...
void makeTemplate(TString channel){
  ...
  makePlot1D( h_D,"MH125To"+channel );
  makePlot2D( h_mzzD,"MH125To"+channel );
  ...
  if (makeAltSignal) {
    low = fillTemplate(channel,3);
    //    high = fillTemplate(channel,3,false);
    h_mzzD =   (TH2F*)low->Clone("h_mzzD");// mergeTemplates(low,high);
    h_D = fillKDhisto(channel,3,105,140);//last two are cuts on mZZ
    // ---------- apply interference reweighting --------
 ...
 //rew for interf currently not available, to be estimated
  ...
  // --------------------------------------------------
    makePlot1D( h_D,"PSMH125To"+channel );
  ...
  low = fillTemplate(channel,1);
  //  high = fillTemplate(channel,1,false);
  h_mzzD =   (TH2F*)low->Clone("h_mzzD");// mergeTemplates(low,high);
  h_D = fillKDhisto(channel,1,105,140);//last two are cuts on mZZ
  ...
  low = fillTemplate(channel,2);
  //  high = fillTemplate(channel,2,false);
  h_mzzD =  (TH2F*)low->Clone("h_mzzD");// mergeTemplates(low,high);
  h_D = fillKDhisto(channel,2,105,140);//last two are cuts on mZZ
