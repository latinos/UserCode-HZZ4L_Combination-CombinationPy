...
void plot_Exclusion_1d2d()
{
  ...
  for(int qq = 0; qq < nME2; qq++)
    {
      ratio[nRatio] = (gr->Eval(a_masses2[qq],0,"S")-gr2D->Eval(a_masses2[qq],0,"S"))/gr->Eval(a_masses2[qq],0,"S");
      nRatio++;
  ...
  sprintf( outfileName,"%s/2DImprovement_%.0fTeV.png",plotDir.c_str(),sqrts );
  //cRatio->SaveAs(outfileName);
  sprintf( outfileName,"%s/2DImprovement_%.0fTeV.eps",plotDir.c_str(),sqrts );
  //cRatio->SaveAs(outfileName);
